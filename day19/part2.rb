...
scanners.first.push({ 'x' => 0, 'y' => 0, 'z' => 0, 'scanner' => true })
cnt = 0
loop do
  new_scanners = []
  skip = []
...
# puts beacons.select { |hsh| hsh['scanner'] == false }
# beacons.select { |hsh| hsh['scanner'] == false }.size
scanners = beacons.select { |hsh| hsh['scanner'] == true }

max = 0
scanners.combination(2).each do |c|
  a = c[0]
  b = c[1]
  dist = (a['x'] - b['x']).abs + (a['y'] - b['y']).abs + (a['z'] - b['z']).abs
